...
export class SimEngine {
  ...
  /** Boot the full simulation chain. Idempotent — safe to call after stop(). */
  ...
  /** Cancel the render loop and dispose all resources. Leaves the engine restartable. */
  stop(): void {
    this.started = false
    this.cancelled = true
  ...
  private async initializePhysics(): Promise<void> {
    try {
      ...
      // Top status RGB LED — actuator (write-only). Hardcoded 'idle' (off) until the
      // RobotStatus state machine exists (ACTUATOR_MODELS.md).
      // Top status RGB LED — repurposes the left_eye mesh.
      if (this.robot) {
        this.topRgb = createTopRgb({
          target: this.robot.leftEye,
          initialStatus: 'running',
          enableCompanionLight: false,
          // Parent the companion PointLight to the unscaled robot root so
