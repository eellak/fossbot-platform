...
export function buildSensorsFolder(
  ...
  // "show overlay" master switch — also controls LDR probes + mic radius
  // since they orbit on the same principle (sensor overlays in 3D world).
  const applyOverlayEnabled = (v: boolean) => {
    viz.setEnabled(v)
    setSensorsViz(v)
    if (extras) {
      extras.setLdrProbesVisible(v)
      extras.setMicRadiusVisible(v)
    }
  }
  ...
  if (extras) {
    ...
    // Store the overlay controller so we can attach the listener to it.
    const overlayCtrl = gui
      .add(state, 'enabled')
      .name('show overlay')
      .onChange((v: boolean) => {
        applyOverlayEnabled(v)
        if (v) {
          ldrCtrl.enable()
