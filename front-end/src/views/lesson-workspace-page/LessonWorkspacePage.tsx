...
import { Alert, Box, Button, Chip, Collapse, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, LinearProgress, Paper, Skeleton, Stack, Tab, Tabs, Tooltip, Typography, useMediaQuery, useTheme } from '@mui/material';
...
export default function LessonWorkspacePage({ previewAppearance = false, courseIdOverride, lessonKeyOverride, previewFixture }: LessonWorkspacePageProps = {}) {
  ...
  const editor = previewAppearance && lesson.editorType === 'python' ? <Box sx={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column' }}><Box sx={{ flex: 1, minHeight: 0 }}>{editorContent}</Box><Stack direction="row" justifyContent="flex-end" alignItems="center" sx={{ minHeight: 48, px: 1.5, py: 0.5, borderTop: '1px solid', borderColor: 'divider', '& .MuiButton-root': { minHeight: 36, py: 0.5 }, '@media (pointer: coarse), (max-width: 768px)': { '& .MuiButton-root': { minHeight: 44 } } }}><SearchBar variant="button" /></Stack></Box> : editorContent;
  ...
  return <Box sx={{ height: previewAppearance ? workspaceShellHeight : undefined, minHeight: previewAppearance ? 0 : 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', overflowX: 'clip', bgcolor: 'background.paper', '& .MuiButton-containedPrimary': { color: theme.palette.getContrastText(theme.palette.primary.main) }, '& .MuiButtonBase-root:focus-visible': { outline: '2px solid', outlineColor: 'text.primary', outlineOffset: 2 }, '@media (pointer: coarse), (max-width: 768px)': { '& .MuiButtonBase-root': { minHeight: 44 }, '& .MuiIconButton-root': { minWidth: 44 } } }}>
    ...
    {previewAppearance && hasEditor && <Box sx={{ px: workspaceLayout.horizontalPadding, py: workspaceLayout.verticalPadding, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
      ...
      <Box sx={{ ml: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap', gap: 1.5 }}>
        ...
        {hasStage && <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
          <Button variant="outlined" startIcon={<IconRefresh size={18} />} onClick={() => resetSimulation(true)}>{t('education.workspace.resetSimulation')}</Button>
          <Button variant="outlined" startIcon={<IconCamera size={18} />} onClick={changeCamera}>{t('education.workspace.changeCamera')}</Button>
        </Box>}
      </Box>
    </Box>}
    <Box sx={{ position: 'relative', display: 'flex', flex: 1, minHeight: 0, overflow: previewAppearance ? 'hidden' : undefined }}>
      ...
      <Box component="main" sx={{ flex: 1, minWidth: 0, minHeight: previewAppearance ? 0 : undefined, p: previewAppearance ? 0 : { xs: 1.5, md: 2 }, display: previewAppearance ? 'flex' : undefined, flexDirection: previewAppearance ? 'column' : undefined }}>
        <Box sx={{ flex: previewAppearance ? 1 : undefined, minHeight: previewAppearance ? 0 : undefined, p: previewAppearance ? workspaceLayout.contentPadding : 0, display: previewAppearance ? 'flex' : undefined, flexDirection: previewAppearance ? 'column' : undefined }}>
          ...
          <Box ref={gridRef} sx={{ position: 'relative', display: 'grid', alignItems: hasEditor ? 'stretch' : 'start', gridTemplateAreas: desktopAreas, gridTemplateColumns: hasWorkPane ? `minmax(280px, ${columnSplit}fr) minmax(360px, ${100 - columnSplit}fr)` : 'minmax(0, 1fr)', gridTemplateRows: hasEditor ? `minmax(${lessonWorkspacePaneDefaults.upperMinHeight}px, ${rowSplit}fr) minmax(${lessonWorkspacePaneDefaults.lowerMinHeight}px, ${100 - rowSplit}fr)` : 'auto', gap: workspaceLayout.paneGap, flex: previewAppearance && hasEditor ? 1 : undefined, height: !previewAppearance && hasEditor ? 'calc(100vh - 210px)' : undefined, minHeight: previewAppearance ? 0 : 'calc(100vh - 210px)' }}>
            <Paper component="section" aria-label={t('education.workspace.instructions')} variant="outlined" sx={{ gridArea: 'instructions', p: { xs: 2, lg: 3 }, overflow: 'auto', borderRightColor: highlightColumns ? 'primary.main' : undefined, borderBottomColor: hasStage && highlightRows ? 'primary.main' : undefined, ...paneBorderTransition }}>{instructions}</Paper>
            {hasStage && <Paper variant="outlined" sx={{ gridArea: 'simulator', position: hasEditor ? 'relative' : 'sticky', top: hasEditor ? undefined : 80, alignSelf: 'start', height: hasEditor ? '100%' : 'clamp(380px, 56vh, 560px)', minHeight: 0, overflow: 'hidden', minWidth: 0, borderLeftColor: highlightColumns ? 'primary.main' : undefined, borderBottomColor: hasEditor && highlightRows ? 'primary.main' : undefined, ...paneBorderTransition }}>{simulator}</Paper>}
            ...
            {hasEditor && <Paper variant="outlined" sx={{ gridArea: 'results', p: previewAppearance ? 0 : 2, minHeight: 0, overflow: 'hidden', borderLeftColor: highlightColumns ? 'primary.main' : undefined, borderTopColor: highlightRows ? 'primary.main' : undefined, ...paneBorderTransition }}>{results}</Paper>}
            {hasWorkPane && <WorkspaceResizeHandle overlay direction="vertical" position={columnSplit} valueNow={columnSplit} valueMin={28} valueMax={65} label={t('education.workspace.resizeColumns')} onPointerDown={beginResize('columns')} onReset={resetPaneSizes} onKeyboardResize={resizePaneWithKeyboard('columns')} onHoverChange={(hovered) => setHoveredResize(hovered ? 'columns' : null)} />}
            {hasEditor && <WorkspaceResizeHandle overlay direction="horizontal" position={rowSplit} crossStart={hasStage ? 0 : columnSplit} valueNow={rowSplit} valueMin={40} valueMax={72} label={t('education.workspace.resizeRows')} onPointerDown={beginResize('rows')} onReset={resetPaneSizes} onKeyboardResize={resizePaneWithKeyboard('rows')} onHoverChange={(hovered) => setHoveredResize(hovered ? 'rows' : null)} />}
            {hasWorkPane && hasEditor && <WorkspaceResizeHandle overlay direction="corner" position={columnSplit} secondaryPosition={rowSplit} label={`${t('education.workspace.resizeColumns')}; ${t('education.workspace.resizeRows')}`} onPointerDown={beginResize('corner')} onReset={resetPaneSizes} onKeyboardResize={resizePaneWithKeyboard('corner')} onHoverChange={(hovered) => setHoveredResize(hovered ? 'corner' : null)} />}
          </Box>}
        </Box>
