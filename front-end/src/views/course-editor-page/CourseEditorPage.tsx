...
import {
  Accordion, AccordionDetails, AccordionSummary, Alert, Box, Button, ButtonBase, Chip, Dialog, DialogActions, DialogContent, DialogTitle,
  Divider, FormControlLabel, IconButton, MenuItem, Paper, Skeleton, Stack, Switch, Tab, Tabs, TextField, Tooltip, Typography,
  useMediaQuery, useTheme,
} from '@mui/material';
import {
  IconAlertTriangle, IconArrowDown, IconArrowLeft, IconArrowUp, IconCheck, IconChevronDown, IconChevronRight, IconCopy, IconEye,
...
export default function CourseEditorPage() {
  ...
  const liveIssues = useMemo(() => localPublicationIssues(course, t), [course, t]);
  ...
  const handleSaveError = (err: unknown, scope: Conflict['scope'], lessonId?: number) => {
    ...
    const serverMessage = err instanceof CourseRequestError && err.message && err.message !== 'Course request failed' ? err.message : '';
    const validationBlocked = err instanceof CourseRequestError && err.status === 422;
    setSaveState('failed');
    setError(validationBlocked ? serverMessage || t('education.errors.saveValidation') : t('education.errors.save'));
    setErrorRecovery('save');
  };
...
function SettingsPanel({ course, lesson, courseChanged, publishedVersion, userLabel, token, tab, issues, liveIssues, highlightedValidation, onTab, onCourse, onLesson, onIssue, t }: any) {
  ...
  return <Stack sx={{ height: '100%' }}><Tabs value={tab} onChange={(_, value) => onTab(value)} variant="scrollable" scrollButtons="auto" allowScrollButtonsMobile sx={{ borderBottom: 1, borderColor: 'divider', minHeight: 44 }}><Tab value="course" aria-label={courseChanged ? t('education.publish.changedSection', { section: t('education.settings.course'), version: publishedVersion }) : undefined} label={<Stack component="span" direction="row" alignItems="center" gap={0.5} sx={{ whiteSpace: 'nowrap' }}><span>{t('education.settings.course')}</span>{courseChanged && <ChangeBadge publishedVersion={publishedVersion} t={t} />}</Stack>} /><Tab value="lesson" label={t('education.settings.lesson')} /><Tab value="validation" label={<Stack component="span" direction="row" alignItems="center" gap={0.5} sx={{ whiteSpace: 'nowrap' }}><span>{t('education.settings.validation')}</span>{shownIssues.length > 0 && <Chip size="small" color="warning" label={shownIssues.length} sx={{ height: 18, '& .MuiChip-label': { px: 0.75, fontSize: '0.6875rem', fontWeight: 600 } }} />}</Stack>} /></Tabs><Box {...(tab === 'validation' ? { 'data-authoring-target': 'validation' } : {})} sx={{ p: 2, overflow: 'auto', '& .MuiInputBase-input': { fontSize: '0.875rem', lineHeight: 1.5 }, ...(tab === 'validation' && highlightedValidation ? authoringTargetOutlineSx : {}) }}>
    {tab === 'course' && <Stack spacing={2.5}>
      <Stack spacing={2}>
        ...
        <Stack direction="row" spacing={0.75} alignItems="baseline" sx={{ minWidth: 0 }}>
          ...
          <Typography variant="body2" noWrap title={userLabel} sx={{ minWidth: 0 }}>{userLabel}</Typography>
        </Stack>
      </Stack>
      <Divider />
      <TextField required multiline minRows={4} size="small" label={t('education.fields.objectives')} error={!course.learning_objectives.some((item: string) => item.trim())} helperText={course.learning_objectives.some((item: string) => item.trim()) ? t('education.fields.objectivesHelp') : t('education.validation.objectiveRequired')} value={course.learning_objectives.join('\n')} onChange={(event) => onCourse({ learning_objectives: event.target.value.split('\n') })} onBlur={() => onCourse({ learning_objectives: course.learning_objectives.map((item: string) => item.trim()).filter(Boolean) })} />
    ...
    {tab === 'validation' && (shownIssues.length === 0 ? <Stack direction="row" alignItems="flex-start" gap={0.75}><Box component="span" sx={{ color: 'success.main', display: 'flex', flexShrink: 0, mt: '2px' }}><IconCheck size={18} /></Box><Box><Typography variant="subtitle2" sx={authoringTitleSx}>{t('education.validation.ready')}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>{t('education.validation.help')}</Typography></Box></Stack> : <Stack divider={<Divider flexItem />}>{shownIssues.map((issue: PublicationIssue, index: number) => <ButtonBase key={`${issue.group}-${index}`} onClick={() => onIssue(issue)} aria-label={`${t('education.validation.go')}: ${issue.message}`} sx={{ width: '100%', justifyContent: 'flex-start', textAlign: 'left', alignItems: 'flex-start', gap: 1.25, px: 0.5, py: 1.5, borderRadius: 1, '&:hover': { bgcolor: 'action.hover' } }}><Box component="span" sx={{ color: 'warning.main', display: 'flex', flexShrink: 0, mt: '2px' }}><IconAlertTriangle size={18} /></Box><Box sx={{ minWidth: 0, flex: 1 }}><Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ display: 'block' }}>{t(`education.validation.groups.${issue.group}`)}</Typography><Typography variant="body2" sx={{ overflowWrap: 'anywhere' }}>{issue.message}</Typography></Box><Box component="span" aria-hidden sx={{ color: 'text.secondary', display: 'flex', flexShrink: 0, mt: '2px' }}><IconChevronRight size={18} /></Box></ButtonBase>)}</Stack>)}
  </Box></Stack>;
}
