...
import {
  Accordion, AccordionDetails, AccordionSummary, Alert, Box, Button, Checkbox, FormControlLabel, FormGroup, IconButton, MenuItem, Paper, Stack, TextField, Typography,
} from '@mui/material';
...
export default function MissionActivityEditor({ activity, onChange, stageReference, token, t }: Props) {
  ...
  useEffect(() => {
    if (!stageReference) { setMarkers([]); setStageError(''); return; }
    let cancelled = false;
    setStageError('');
    ...
    request.then((entries) => { if (!cancelled) setMarkers(readMarkers(entries || [])); })
      .catch((error) => { if (!cancelled) { setMarkers([]); setStageError(error instanceof Error ? error.message : String(error)); } });
  ...
  const addObjective = () => patch({ objectives: [...activity.objectives, objectiveFor('reach_target', 'optional', markerGroups, t)] });
  const applyTemplate = () => {
    ...
    patch({
      ...
      objectives: definitions[template].map(([type, role]) => objectiveFor(type, role, markerGroups, t)),
    });
  };
  ...
  return <Stack spacing={2}>
    ...
    <Paper variant="outlined" sx={{ p: 1.5 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
        <TextField select fullWidth size="small" label={t('education.mission.template')} value={template} onChange={(event) => setTemplate(event.target.value as typeof template)}>
          {templates.map((item) => <MenuItem key={item} value={item}>{t(`education.mission.templates.${item}`)}</MenuItem>)}
        </TextField>
        <Button variant="outlined" onClick={applyTemplate}>{t('education.mission.useTemplate')}</Button>
      </Stack>
    </Paper>
    {stageError && <Alert severity="warning">{t('education.mission.stageUnavailable')} {stageError}</Alert>}
    {!stageError && markers.length === 0 && <Alert severity="warning">{t('education.mission.noMarkers')}</Alert>}
    {!stageError && missingReferences.length > 0 && <Alert severity="error">{t('education.mission.invalidReferences', { ids: missingReferences.join(', ') })}</Alert>}
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
    ...
    {activity.objectives.map((objective, index) => <Paper key={objective.key} variant="outlined" sx={{ p: 1.5 }}>
      <Stack spacing={1.5}>
        <Stack direction="row" alignItems="center" gap={1}>
          <Typography fontWeight={700} sx={{ flex: 1 }}>{t('education.mission.objective', { number: index + 1 })}</Typography>
          <IconButton disabled={activity.objectives.length === 1} onClick={() => patch({ objectives: activity.objectives.filter((item) => item.key !== objective.key) })} aria-label={t('education.activities.delete')}><IconTrash size={17} /></IconButton>
        </Stack>
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
        ...
        <Alert severity={objective.role === 'failure' ? 'warning' : objective.role === 'optional' ? 'info' : 'success'}>{objective.summary}</Alert>
      </Stack>
    </Paper>)}
  ...
  return <Accordion variant="outlined" disableGutters>
    <AccordionSummary expandIcon={<IconChevronDown size={18} />} aria-controls="mission-authoring-guide-content" id="mission-authoring-guide-header" sx={{ '& .MuiAccordionSummary-content': { my: 1.25 } }}>
      <Stack direction="row" spacing={1.25} alignItems="center">
        <IconBook2 size={20} />
        <Box><Typography fontWeight={700}>{t('education.mission.guide.title')}</Typography><Typography variant="caption" color="text.secondary">{t('education.mission.guide.summary')}</Typography></Box>
      </Stack>
    </AccordionSummary>
    <AccordionDetails id="mission-authoring-guide-content" sx={{ borderTop: '1px solid', borderColor: 'divider' }}>
      <Stack spacing={2}>
        <Alert severity="info">{t('education.mission.guide.principle')}</Alert>
        ...
        <Box>
          ...
          <Box component="dl" sx={{ m: 0 }}>
            {conditionTypes.map((type, index) => <Box key={type} sx={{ py: 0.75, borderTop: index ? '1px solid' : 0, borderColor: 'divider' }}>
              <Typography component="dt" variant="body2" fontWeight={700}>{t(`education.mission.conditions.${type}`)}</Typography>
...
function GuideSection({ title, body }: { title: string; body: string }) {
  return <Box><Typography variant="subtitle2">{title}</Typography><Typography variant="body2" color="text.secondary">{body}</Typography></Box>;
}
...
function ConditionFields({ condition, markerGroups, onChange, t }: { condition: MissionCondition; markerGroups: Record<MarkerKind, Marker[]>; onChange: (condition: MissionCondition) => void; t: any }) {
  ...
  if (condition.type === 'actuator_state') {
    ...
    return <Stack direction="row" spacing={1}><TextField select label={t('education.mission.actuator')} value={condition.actuator} onChange={(event) => { const actuator = event.target.value as 'led' | 'buzzer'; onChange({ ...condition, actuator, state: actuator === 'led' ? 'green' : 'on' }); }}><MenuItem value="led">LED</MenuItem><MenuItem value="buzzer">{t('education.mission.buzzer')}</MenuItem></TextField><TextField select label={t('education.mission.state')} value={condition.state} onChange={(event) => onChange({ ...condition, state: event.target.value })}>{states.map((state) => <MenuItem key={state} value={state}>{state}</MenuItem>)}</TextField></Stack>;
  }
...
function MarkerSelect({ label, value, markers, onChange }: { label: string; value: string; markers: Marker[]; onChange: (value: string) => void }) {
  return <TextField select fullWidth label={label} value={value} onChange={(event) => onChange(event.target.value)}>{markers.map((marker) => <MenuItem key={marker.id} value={marker.id}>{marker.name} · {marker.id}</MenuItem>)}</TextField>;
}

function MarkerChecks({ markers, selected, onChange }: { markers: Marker[]; selected: string[]; onChange: (values: string[]) => void }) {
  return <FormGroup row>{markers.map((marker) => <FormControlLabel key={marker.id} control={<Checkbox checked={selected.includes(marker.id)} onChange={(event) => onChange(event.target.checked ? [...selected, marker.id] : selected.filter((id) => id !== marker.id))} />} label={`${marker.name} · ${marker.id}`} />)}</FormGroup>;
}
