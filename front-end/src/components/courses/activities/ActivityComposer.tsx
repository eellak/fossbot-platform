...
import {
  Alert, Box, Button, Checkbox, Divider, FormControlLabel, FormGroup, IconButton, MenuItem, Paper, Stack, Switch, TextField, Typography,
} from '@mui/material';
...
export default function ActivityComposer({ activities, onChange, stageReference, token, t }: Props) {
  ...
  return <Stack spacing={2}>
    ...
    <Paper variant="outlined" sx={{ p: 1.5 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
      <TextField select fullWidth size="small" label={t('education.activities.addType')} value={newType} onChange={(event) => setNewType(event.target.value as Activity['type'])}>{activityTypes.map((type) => <MenuItem key={type} value={type}>{type === 'hint' ? t('education.activities.hintGeneralActivity') : t(`education.activities.types.${type}`)}</MenuItem>)}</TextField>
      <Button variant="outlined" startIcon={<IconPlus size={17} />} onClick={() => onChange([...activities, createActivity(newType)])}>{t('education.activities.add')}</Button>
    </Stack></Paper>
  </Stack>;
}
...
function ActivityCard({ activity, linkedHints, index, count, onChange, onHintsChange, onMove, onDuplicate, onDelete, stageReference, token, t }: { activity: Activity; linkedHints: HintActivity[]; index: number; count: number; onChange: (activity: Activity) => void; onHintsChange: (hints: HintActivity[]) => void; onMove: (direction: -1 | 1) => void; onDuplicate: () => void; onDelete: () => void; stageReference?: StageReference | null; token?: string; t: any }) {
  ...
  return <Paper component="section" variant="outlined" aria-labelledby={`activity-${activity.key}`} sx={{ p: 2 }}>
    <Stack direction="row" alignItems="center" spacing={1}>
      <Box sx={{ flex: 1 }}><Typography id={`activity-${activity.key}`} fontWeight={700}>{index + 1}. {activity.type === 'hint' ? t('education.activities.hintGeneralActivity') : t(`education.activities.types.${activity.type}`)}</Typography><Typography variant="caption" color="text.secondary">{activity.key}</Typography></Box>
      <IconButton size="small" disabled={index === 0} onClick={() => onMove(-1)} aria-label={t('education.activities.moveUp')}><IconArrowUp size={17} /></IconButton>
      <IconButton size="small" disabled={index === count - 1} onClick={() => onMove(1)} aria-label={t('education.activities.moveDown')}><IconArrowDown size={17} /></IconButton>
      <IconButton size="small" onClick={onDuplicate} aria-label={t('education.activities.duplicate')}><IconCopy size={17} /></IconButton>
      <IconButton size="small" color="error" onClick={onDelete} aria-label={t('education.activities.delete')}><IconTrash size={17} /></IconButton>
    </Stack>
    <Divider sx={{ my: 1.5 }} />
...
function ActivityFields({ activity, onChange, stageReference, token, t }: { activity: Activity; onChange: (activity: Activity) => void; stageReference?: StageReference | null; token?: string; t: any }) {
  ...
  if (activity.type === 'hint') {
    return <Stack spacing={1.5}>
      <Alert severity="info">{t('education.activities.hintGeneralHelp')}</Alert>
...
function ActivityHintFields({ activityKey, hints, onChange, t }: { activityKey: string; hints: HintActivity[]; onChange: (hints: HintActivity[]) => void; t: any }) {
  ...
  return <Box sx={{ mt: 1.5, pt: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
...
function ChoiceFields({ activity, onChange, t }: { activity: Extract<Activity, { type: 'multiple_choice' | 'multiple_select' }>; onChange: (activity: Activity) => void; t: any }) {
  ...
  return <Stack spacing={1.5}>
    <TextField required fullWidth multiline label={t('education.activities.prompt')} value={activity.prompt} onChange={(event) => onChange({ ...activity, prompt: event.target.value })} />
    <Typography variant="subtitle2">{t('education.activities.options')}</Typography>
    {activity.options.map((option, index) => <Stack key={option.key} direction="row" spacing={1} alignItems="center">
      <Checkbox checked={activity.type === 'multiple_choice' ? activity.correctOptionKey === option.key : Boolean(activity.correctOptionKeys?.includes(option.key))} onChange={(event) => toggleCorrect(option.key, event.target.checked)} inputProps={{ 'aria-label': t('education.activities.correctOption', { number: index + 1 }) }} />
      ...
      <IconButton disabled={activity.options.length <= 2} onClick={() => removeOption(option.key)} aria-label={t('education.activities.deleteOption')}><IconTrash size={17} /></IconButton>
    </Stack>)}
...
function ObservationFields({ activity, onChange, t }: { activity: Extract<Activity, { type: 'simulator_observation' }>; onChange: (activity: Activity) => void; t: any }) {
  ...
  return <Stack spacing={1.5}>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5 }}>
      {sensorGroups.map((group) => <Box component="fieldset" key={group} sx={{ m: 0, p: 1.25, minWidth: 0, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}><Typography component="legend" variant="caption" fontWeight={700} sx={{ px: 0.5 }}>{t(`education.activities.sensorGroups.${group}`)}</Typography><FormGroup>{sensorCatalog.filter((sensor) => sensor.group === group).map((sensor) => <FormControlLabel key={sensor.id} control={<Checkbox checked={activity.allowedSensors.includes(sensor.id)} onChange={(event) => onChange({ ...activity, allowedSensors: toggle(activity.allowedSensors, sensor.id, event.target.checked) })} />} label={`${t(`education.sensors.${sensor.id}`, sensor.id)} (${sensor.unit})`} />)}</FormGroup></Box>)}
    </Box>
