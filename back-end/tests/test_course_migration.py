...
def test_legacy_curriculum_and_lesson_survive_upgrade(tmp_path):
    ...
    assert {"courses", "course_releases", "enrollments", "lesson_progress", "lesson_workspaces", "activity_answers", "mission_attempts", "class_groups", "class_memberships", "course_assignments", "class_challenges", "local_stages", "local_marketplace_submissions", "local_marketplace_publications"}.issubset(
    ...
    assert "stage_local_id" in {column["name"] for column in inspector.get_columns("lessons")}
