...
export function App() {
  ...
  useEffect(() => {
    ...
    const positionPresets = createPositionPresets(containerRef.current, {
      save: (name: string) => {
        if (!robotPhysics) return
        const pos = robotPhysics.body.translation()
        const rot = robotPhysics.body.rotation()
        positionStore.save(name, { x: pos.x, y: pos.y, z: pos.z }, { x: rot.x, y: rot.y, z: rot.z, w: rot.w })
      },
      load: (name: string) => {
        if (!robotPhysics) return
        const saved = positionStore.load(name)
        if (!saved) return
        robotPhysics.body.setTranslation(saved.position, true)
        robotPhysics.body.setRotation(saved.rotation, true)
        robotPhysics.body.setLinvel({ x: 0, y: 0, z: 0 }, true)
        robotPhysics.body.setAngvel({ x: 0, y: 0, z: 0 }, true)
        robotPhysics.body.wakeUp()
      },
      deletePos: (name: string) => {
        positionStore.remove(name)
      },
      getSavedNames: () => positionStore.list(),
    })
    ...
    const swapStage = async (next: StageName) => {
      ...
      positionStore.setStage(next) // load + persist position presets for the new stage
      positionPresets.refresh()
    ...
    const initializePhysics = async () => {
      try {
        ...
        positionStore.setStage(initialStage)
        positionPresets.refresh()
