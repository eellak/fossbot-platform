...
export function createSplashScreen(container: HTMLElement, enabled: boolean, opts: SplashScreenOptions = {}): SplashScreenHandle {
  let removed = false
  ...
  if (splash) {
    ...
    splash.innerHTML = `
      <div style="text-align:center; padding:24px 28px; border:1px solid rgba(255,255,255,0.14); border-radius:14px; background:rgba(0,0,0,0.26); box-shadow:0 18px 60px rgba(0,0,0,0.35);">
        <img src="${opts.logoUrl ?? '/images/superlogo.png'}" alt="Fossbot" style="width:300px; height:auto; margin-bottom:14px; display:block; margin-left:auto; margin-right:auto;" />
